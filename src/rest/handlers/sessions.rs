...
use crate::models::{AppState, Session, SessionState, CreateSessionRequest, RemixSessionRequest, UpdateSessionStateRequest, UpdateSessionRequest};
...
pub async fn create_session(
    ...
    let mut session = Session::create(&state.db, req.clone(), username.clone())
        .await
        .map_err(|e| {
    ...
    // Create Docker container if service is configured
    if let Some(docker) = &state.docker {
        match docker.create_session_container(&session).await {
            Ok(container_id) => {
                session.container_id = Some(container_id);
                tracing::info!("Created Docker container for session {}", session.id);
            }
            Err(e) => {
                tracing::error!("Failed to create Docker container for session {}: {}", session.id, e);
                // Continue without container - session is still created in database
            }
        }
    }
...
pub async fn update_session_state(
    ...
    if let Some(docker) = &state.docker {
        // Handle state transitions that affect containers
        match (old_state, new_state) {
            ...
            (SessionState::Ready, SessionState::Idle) => {
                ...
                if let Err(e) = docker.stop_session_container(session_id).await {
                    tracing::error!("Failed to stop container for idle session {}: {}", session_id, e);
                }
            }
...
pub async fn delete_session(
    ...
    // Delete Docker container if service is configured
    if let Some(docker) = &state.docker {
        if let Err(e) = docker.remove_session_container(session_id).await {
            tracing::error!("Failed to delete Docker container for session {}: {}", session_id, e);
            // Continue with session deletion even if container deletion fails
        } else {
            tracing::info!("Deleted Docker container for session {}", session_id);
        }
    }
