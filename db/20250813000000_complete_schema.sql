...
-- Session Tasks table for task queue
...
CREATE INDEX idx_session_tasks_status ON session_tasks(status) WHERE status = 'pending';
CREATE INDEX idx_session_tasks_session_id ON session_tasks(session_id);
CREATE INDEX idx_session_tasks_created_at ON session_tasks(created_at);

-- Command Results table for storing command execution results
...
CREATE INDEX idx_command_results_session_id ON command_results(session_id);
